...
|  3️⃣  | **조건을 만족하는 모든 경로 찾기**                           | "몇 가지 방법이 있나?", "경로 수는?"   | DFS + DP (Top-down), 위상정렬 + DP     | 내리막길, 파이프 옮기기, DAG 경로 수 구하기      |
|  4️⃣  | [**구조 구성 / 연결성 판별**](../graph_theory/README.md) | "이 구조가 가능한가?", "어떻게 구성할까?" | 유니온 파인드, MST(크루스칼/프림), 위상정렬, 이분 매칭 | 친구 네트워크, 작업 순서, 팀 구성, 이분 그래프 확인  |
