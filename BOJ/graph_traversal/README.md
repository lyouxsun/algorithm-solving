...
| 19 | 24.08.26 | [BOJ 17086](https://www.acmicpc.net/problem/17086) | BFS            | 그래프 탐색 문제에서는 모든 위치에서의 최단 거리를 계산하지 말고, 변하지 않는 것들(문제의 주어진 input)을 가지고 최소의 계산으로 문제를 푸는 방법을 찾아보자! | -> 이게 뭔 말인지 나도 잘 모르겠지만.. 아무튼 시간복잡도를 최소화하는 코드를 짜라는 뜻임!                                 |         |
...
| 21 | 24.08.27 | [BOJ 4991](https://www.acmicpc.net/problem/4991)   | BFS            | BFS+메모리제이션(DP) 인데,, 접근 방식에 따라서도 시간초과가 나는 까다로운 문제 (ꐦ°᷄▿°᷅)                                     | 문자열 리스트를 입력받을 때에는 반드시 줄바꿈문자 제거해줘야함! `.strip()`                                        |         |
| 22 | 24.08.28 | [BOJ 2151](https://www.acmicpc.net/problem/2151)   | BFS            | 이 문제의 핵심은 이동 방향에 따른 각 좌표의 최소 거울개수(cnt)를 배열에 저장하기! `visited`                                   | 참.. 어렵네요..ㅎ                                                                           |         |
